interface calc_if #(int DataSize, int AddrSize) (input wire clk);
  ...
  logic [DataSize-1:0] wr_data_lower;
  logic [DataSize-1:0] wr_data_upper;
  ...
  logic [DataSize-1:0] rd_data_lower;
  logic [DataSize-1:0] rd_data_upper;
...
clocking cb @(posedge clk);
  ...
  input ready, rd_data_lower, rd_data_upper, wr_en, rd_en, wr_data_lower, wr_data_upper, curr_rd_addr, curr_wr_addr, loc_sel;
  output reset;
...
modport calc(
  input reset, wr_en, rd_en, wr_data_lower, wr_data_upper, read_start_addr, read_end_addr, write_start_addr, write_end_addr, curr_rd_addr, curr_wr_addr, loc_sel,
  output ready, rd_data_lower, rd_data_upper
);
